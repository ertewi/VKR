...
public class IronSeamFeature extends Feature<OreConfiguration> {

    public IronSeamFeature(Codec<OreConfiguration> codec) {
        super(codec);
    }
    ...
    public boolean place(FeaturePlaceContext<OreConfiguration> context) {
        ...
        int placed = generateIronSeam(level, random, config, origin);
        return placed > 0;
    }

    private int generateIronSeam(
            WorldGenLevel level,
            RandomSource random,
            ...
            for (int x = -seamRadius; x <= seamRadius; x++) {
                for (int z = -seamRadius; z <= seamRadius; z++) {
                    ...
                    for (int yOffset = -seamThickness; yOffset <= seamThickness; yOffset++) {
                        ...
                        if (rand < 0.05) {
                            placeBlock = Blocks.RAW_IRON_BLOCK.defaultBlockState();
                        } else if (rand < 0.9) {
                            placeBlock = getIronOreState(currentBlock);
                        } else {
                            placeBlock = null;
                        }
    ...
    private BlockState getIronOreState(BlockState current) {
